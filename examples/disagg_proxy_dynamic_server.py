...
class AdaptiveBucketingSchedulingPolicy(SchedulingPolicy):
    ...
    def __init__(self, l_max: int = 1000, n_max: int = 100, theta: float = 0.5,
        ...
        logger.info(
            f"Initialized adaptive bucketing with optimized load balancing: L_max={l_max}, N_max={n_max}, θ={theta}, max_active_per_instance={max_active_requests_per_instance}, priority_threshold={priority_threshold}")
    ...
    def _assign_instance_to_bucket(self, bucket, instance_type: str, cycler: itertools.cycle) -> str:
        ...
        logger.debug(
            f"Assigned {instance_type} instance {assigned_instance} to bucket [{bucket['low']}, {bucket['up']})")
        ...
        with self._lock:
            for bucket_idx, bucket in enumerate(self.buckets):
                if bucket['low'] <= sequence_length < bucket['up']:
                    ...
                    logger.debug(
                        f"Added request {request_id} (length={sequence_length}) to bucket [{bucket['low']}, {bucket['up']})")
                    break
                ...
                if bucket['low'] <= sequence_length < bucket['up']:
                    ...
                    if instance_key in bucket['instance_mapping']:
                        ...
                        if current_load >= self.max_active_requests_per_instance:
                            logger.debug(
                                f"Instance {assigned_instance} overloaded (load={current_load}), using round-robin for request with length {sequence_length}")
                            return next(cycler)
                        else:
                            # Instance is not overloaded, use bucket's assigned instance
                            if is_priority:
                                logger.debug(
                                    f"Priority scheduling: shorter request (length={sequence_length}) assigned to {assigned_instance}")
                            return assigned_instance
                    else:
                        ...
                        if is_priority:
                            logger.debug(
                                f"Priority scheduling: first request for bucket (length={sequence_length}) assigned to {assigned_instance}")
                        return assigned_instance
...
class SimplePrioritySchedulingPolicy(SchedulingPolicy):
    """
    ...
    def __init__(self, max_active_requests_per_instance: int = 3, stats_max_count: int = 100000):
        self.max_active_requests_per_instance = max_active_requests_per_instance
        self.stats_max_count = stats_max_count  # 统计上限
        self.instance_active_requests = {}  # instance -> active request count
        self.instance_performance_history = {}  # instance -> list of recent response times
        self.request_length_stats = {'count': 0, 'sum': 0}
        self.request_length_list = []  # 存储序列长度列表，用于计算分位数
        self._lock = threading.Lock()
        logger.info(
            f"Initialized intelligent priority scheduling: max_active_per_instance={max_active_requests_per_instance}, stats_max_count={stats_max_count}")
    ...
    def _update_request_length_stats(self, sequence_length: int):
        """Update global request length statistics with simple reset mechanism"""
        with self._lock:
            ...
            if len(self.request_length_list) >= self.stats_max_count:
                ...
                self.request_length_stats = {'count': 0, 'sum': 0}
                self.request_length_list.clear()  # 清空列表
        ...
        with self._lock:
            if request_id in self.request_history:
                ...
                if status == RequestStatus.PREFILLING:
                    ...
                    if assigned_instance and isinstance(self.scheduling_policy, SimplePrioritySchedulingPolicy):
                        self.scheduling_policy.record_response_time(assigned_instance, total_time)
                        logger.debug(f"Recorded response time {total_time:.3f}s for instance {assigned_instance}")

                # Update status in scheduling policy
                ...
                elif isinstance(self.scheduling_policy, SimplePrioritySchedulingPolicy):
...
class xPyDProxyServer:
    ...
    def __init__(
        ...
        if scheduling_policy is None:
            ...
            elif args.scheduling == "simple_priority":
                scheduling_policy = SimplePrioritySchedulingPolicy(
                    max_active_requests_per_instance=args.max_active_requests_per_instance,
                    stats_max_count=args.stats_max_count
                )
            else:
...
def parse_args():
    ...
    parser.add_argument(
        "--scheduling",
        type=str,
        choices=["round_robin", "adaptive_bucketing", "simple_priority"],
        default="simple_priority",
    ...
    # Simple priority scheduling parameters
    parser.add_argument(
        "--stats-max-count",
        type=int,
        default=100000,
        help="Maximum count for simple priority scheduling statistics before reset",
    )
